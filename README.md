...
备注说明：[./data/medfw.txt](./data/medfw.txt) 文件是我上个项目`find-Chinese-medcial-words`在同样语料库找出的词文件，本次作为用户词库参与jieba分词; [similarity.py](./similarity.py)文件为四种算法实现的程序，可以调用，不同的环境下只需重新训练词向量和词的idf矩阵；[./data/test_result.txt](./data/test_result.txt)文件是本实验测试结果。<br>
